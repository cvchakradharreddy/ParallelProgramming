...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: coll.Seq[Body])
    extends Quad {
    val mass = (bodies.foldLeft(0.0f)((acc, b) => acc + b.mass))
    val massX =
      if (mass != 0) bodies.foldLeft(0.0f)((acc, b) => acc + b.mass * b.x) / mass
      else centerX
    val massY =
      if (mass != 0) bodies.foldLeft(0.0f)((acc, b) => acc + b.mass * b.y) / mass
      else centerY
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case Fork(nw, ne, sw, se) =>
          ...
          if (quad.size / distance(quad.massX, quad.massY, quad.centerX, quad.centerY) < theta)
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface {
    ...
    def +=(b: Body): SectorMatrix = {
      ...
      val posX = if (x < 0) 0 else if (x > sectorPrecision) sectorPrecision else x
      val posY = if (y < 0) 0 else if (y > sectorPrecision) sectorPrecision else y
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      ...
      (0 until matrix.length).foreach(i =>
