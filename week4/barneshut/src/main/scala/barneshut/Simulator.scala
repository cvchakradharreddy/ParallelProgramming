...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    ...
    newBoundaries.minX = if (a.minX <= b.minX) a.minX else b.minX
    newBoundaries.maxX = if (a.maxX >= b.maxX) a.maxX else b.maxX
    newBoundaries.minY = if (a.minY <= b.minY) a.minY else b.minY
    newBoundaries.maxY = if (a.maxY >= b.maxY) a.maxY else b.maxY
    newBoundaries
  }
  ...
  def updateBodies(bodies: coll.Seq[Body], quad: Quad): coll.Seq[Body] = timeStats.timed("update") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.map(_.updated(quad)).to(Seq)
  }
