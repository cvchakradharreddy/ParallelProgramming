...
object VerticalBoxBlur extends VerticalBoxBlurInterface {
  ...
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit = {
    ...
    val splits = (0 to src.width by math.ceil(1.0 * src.width / numTasks).toInt).toList
    val intervals = if (splits.last < src.width) splits.zip(splits.tail) :+ (splits.last, src.width) else splits.zip(splits.tail)
